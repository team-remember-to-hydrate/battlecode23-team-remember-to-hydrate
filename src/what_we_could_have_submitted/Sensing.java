...
import what_we_could_have_submitted.Comms;
import what_we_could_have_submitted.RobotPlayer;
import what_we_could_have_submitted.RobotPlayer.map_tiles;
...
public class Sensing {
    ...
    public static RobotInfo[] smartScanMembersOfTeam(RobotController rc, Team team, int range) throws GameActionException {
        if (team == rc.getTeam().opponent()) {
            if (what_we_could_have_submitted.RobotPlayer.lastRoundScannedEnemies != rc.getRoundNum()){
                // Our old scan is outdated, do a new one.
                what_we_could_have_submitted.RobotPlayer.scannedEnemies = rc.senseNearbyRobots(range, team); //102 bc
                what_we_could_have_submitted.RobotPlayer.lastRoundScannedEnemies = rc.getRoundNum();
            }
                // Just use our existing scan.
                return what_we_could_have_submitted.RobotPlayer.scannedEnemies;
        }
        else {
            if (what_we_could_have_submitted.RobotPlayer.lastRoundScannedAllies != rc.getRoundNum()){
                // Our old scan is outdated, do a new one.
                what_we_could_have_submitted.RobotPlayer.scannedAllies = rc.senseNearbyRobots(range, team); //102 bc
                what_we_could_have_submitted.RobotPlayer.lastRoundScannedAllies = rc.getRoundNum();
            }
            // Just use our existing scan.
            return what_we_could_have_submitted.RobotPlayer.scannedAllies;
        }
    }
    ...
    static int makeIslandBroadcastPair(RobotController rc, MapLocation location, boolean friendly_owned,
        ...
        int compressedStrength = what_we_could_have_submitted.Comms.compressCount(ownerCombatStrength);
        ...
        int compressedFriendlies = what_we_could_have_submitted.Comms.compressCount(friendlies);
        packedDetails += compressedFriendlies  << 6;
        int compressedEnemies = what_we_could_have_submitted.Comms.compressCount(enemies);
    ...
    static void scanAndUpdateIslands(RobotController rc) throws GameActionException {
        what_we_could_have_submitted.RobotPlayer.myCurrentLocation = rc.getLocation();
        // Scan Island Info and Queue/Report Changes - 200 bc + potentially over 100 per island in range
        if (what_we_could_have_submitted.RobotPlayer.scannedIslandIDs == null
                || (what_we_could_have_submitted.RobotPlayer.myCurrentLocation != what_we_could_have_submitted.RobotPlayer.lastLocationScannedIslands)) {
            what_we_could_have_submitted.RobotPlayer.scannedIslandIDs = rc.senseNearbyIslands();
            what_we_could_have_submitted.RobotPlayer.lastLocationScannedIslands = rc.getLocation();
            if (Clock.getBytecodesLeft() > 1000){
                Team ourTeam = rc.getTeam();
                int friendlies = scanCombatUnitsOfTeam(rc, ourTeam).size();
                int enemies = scanCombatUnitsOfTeam(rc, ourTeam.opponent()).size();
                for (int i = 0; i < what_we_could_have_submitted.RobotPlayer.scannedIslandIDs.length; i++){
                    if (Clock.getBytecodesLeft() > 1000){
                        int id = what_we_could_have_submitted.RobotPlayer.scannedIslandIDs[i];
                        ...
                        if (what_we_could_have_submitted.RobotPlayer.island_locations[id] == null){
                            ...
                            if (what_we_could_have_submitted.Comms.set_island_from_island_broadcast_pair(rc, islandBroadcastPair)){
                                // It has now been written to the comm array
                                what_we_could_have_submitted.RobotPlayer.island_locations[id] = islandLocations[0];
                                what_we_could_have_submitted.RobotPlayer.teamKnownIslandDetails[id] =
                                        islandDetailsFromBroadcastPair(islandBroadcastPair);
                            }
                            else{
                                // Store this until we can broadcast
                                what_we_could_have_submitted.RobotPlayer.myIslandFullInfoBroadcastQueue.add(islandBroadcastPair);
                            }
                        }
                        else {
