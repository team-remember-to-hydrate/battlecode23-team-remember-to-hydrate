...
//        RobotInfo[] nearby_bots = rc.senseNearbyRobots();
        ...
        int my_task = what_we_could_have_submitted.Comms.get_command_for_me(rc);

        if(my_task > 0){
            target_location = what_we_could_have_submitted.Comms.get_MapLocation(my_task);
            int desired_task = what_we_could_have_submitted.Comms.get_task_type(my_task);
