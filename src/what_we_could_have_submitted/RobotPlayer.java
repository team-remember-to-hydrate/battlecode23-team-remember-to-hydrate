...
import what_we_could_have_submitted.Amplifier;
import what_we_could_have_submitted.CarrierStrategy;
import what_we_could_have_submitted.Comms;
import what_we_could_have_submitted.Headquarters;
import what_we_could_have_submitted.Launcher;
...
//        System.out.println("Start of RobotPlayer while loop bc remaining: " + Clock.getBytecodesLeft());
    ...
    static void process_array_islands(RobotController rc) throws GameActionException {
        for(int i = what_we_could_have_submitted.Comms.index_island; i < what_we_could_have_submitted.Comms.index_last_island ; i++){
            int this_island_id;
            MapLocation this_island_location;

            boolean is_location = what_we_could_have_submitted.Comms.is_location(rc.readSharedArray(i));
            int id_offset = is_location ? 1 : 0;
            this_island_id = what_we_could_have_submitted.Comms.get_island_id(rc.readSharedArray(i + id_offset));

            if(island_ids.contains(this_island_id) & rc.getType().equals(RobotType.HEADQUARTERS)){
                what_we_could_have_submitted.Comms.clear_island_location(rc, i, is_location);
            }else {
                this_island_location = what_we_could_have_submitted.Comms.get_MapLocation(rc.readSharedArray(i));
                if(is_location){
    ...
    static void process_array_wells(RobotController rc) throws GameActionException{
        for(int i = what_we_could_have_submitted.Comms.index_last_well; i >= what_we_could_have_submitted.Comms.index_well; i--){
            MapLocation this_well_location;
            ResourceType this_well_type;

            int raw_well_data = rc.readSharedArray(i);
            this_well_location = what_we_could_have_submitted.Comms.get_MapLocation(raw_well_data);
            this_well_type = what_we_could_have_submitted.Comms.get_well_type(raw_well_data);
