    ...
    static void runHeadquarters(RobotController rc) throws GameActionException {
        ...
        if(rc.getRoundNum() == 200){
            ...
            if(my_array_address == 0){
                for(int i = what_we_could_have_submitted.Comms.index_well; i <= what_we_could_have_submitted.Comms.index_last_well; i++){
                    int this_well = rc.readSharedArray(i);
                    if(this_well != 0){
                        System.out.println("Well at " + what_we_could_have_submitted.Comms.unpack_maplocation(this_well) + " of type " + what_we_could_have_submitted.Comms.get_task_type(this_well));
                    }
                }
            }
        ...
        if(!my_recent_tasks.isEmpty()){
            for(int task : my_recent_tasks){
                what_we_could_have_submitted.Comms.clear_command(rc,task);
            }
        }
        ...
        if(num_launchers >= 6 & command_decay < 1){
            ...
            what_we_could_have_submitted.Comms.send_command(rc,center,RobotType.HEADQUARTERS.visionRadiusSquared,1,1, false,false,1,true);
        }
    ...
    static void performRoundOneTasks(RobotController rc)throws GameActionException{
        ...
        if(wells.length > 0){
            ...
            for(WellInfo well : wells){
                int available_index = what_we_could_have_submitted.Comms.get_available_well_index(rc);
