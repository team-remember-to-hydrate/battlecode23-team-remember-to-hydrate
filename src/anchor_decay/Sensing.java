...
import anchor_decay.RobotPlayer.map_tiles;
...
public class Sensing {
    ...
    public static RobotInfo[] smartScanMembersOfTeam(RobotController rc, Team team, int range) throws GameActionException {
        if (team == rc.getTeam().opponent()) {
            if (anchor_decay.RobotPlayer.lastRoundScannedEnemies != rc.getRoundNum()){
                // Our old scan is outdated, do a new one.
                anchor_decay.RobotPlayer.scannedEnemies = rc.senseNearbyRobots(range, team); //102 bc
                anchor_decay.RobotPlayer.lastRoundScannedEnemies = rc.getRoundNum();
            }
                // Just use our existing scan.
                return anchor_decay.RobotPlayer.scannedEnemies;
        }
        else {
            if (anchor_decay.RobotPlayer.lastRoundScannedAllies != rc.getRoundNum()){
                // Our old scan is outdated, do a new one.
                anchor_decay.RobotPlayer.scannedAllies = rc.senseNearbyRobots(range, team); //102 bc
                anchor_decay.RobotPlayer.lastRoundScannedAllies = rc.getRoundNum();
            }
            // Just use our existing scan.
            return anchor_decay.RobotPlayer.scannedAllies;
        }
    }
    ...
    static int makeIslandBroadcastPair(RobotController rc, MapLocation location, boolean friendly_owned,
        ...
        int compressedStrength = anchor_decay.Comms.compressCount(ownerCombatStrength);
        ...
        int compressedFriendlies = anchor_decay.Comms.compressCount(friendlies);
        packedDetails += compressedFriendlies  << 6;
        int compressedEnemies = anchor_decay.Comms.compressCount(enemies);
    ...
    static void scanAndUpdateIslands(RobotController rc) throws GameActionException {
        anchor_decay.RobotPlayer.myCurrentLocation = rc.getLocation();
        // Scan Island Info and Queue/Report Changes - 200 bc + potentially over 100 per island in range
        if (anchor_decay.RobotPlayer.scannedIslandIDs == null
                || (anchor_decay.RobotPlayer.myCurrentLocation != anchor_decay.RobotPlayer.lastLocationScannedIslands)) {
            anchor_decay.RobotPlayer.scannedIslandIDs = rc.senseNearbyIslands();
            anchor_decay.RobotPlayer.lastLocationScannedIslands = rc.getLocation();
            if (Clock.getBytecodesLeft() > 1000){
                Team ourTeam = rc.getTeam();
                int friendlies = scanCombatUnitsOfTeam(rc, ourTeam).size();
                int enemies = scanCombatUnitsOfTeam(rc, ourTeam.opponent()).size();
                for (int i = 0; i < anchor_decay.RobotPlayer.scannedIslandIDs.length; i++){
                    if (Clock.getBytecodesLeft() > 1000){
                        int id = anchor_decay.RobotPlayer.scannedIslandIDs[i];
                        ...
                        if (anchor_decay.RobotPlayer.island_locations[id] == null){
                            ...
                            if (anchor_decay.Comms.set_island_from_island_broadcast_pair(rc, islandBroadcastPair)){
                                // It has now been written to the comm array
                                anchor_decay.RobotPlayer.island_locations[id] = islandLocations[0];
                                anchor_decay.RobotPlayer.teamKnownIslandDetails[id] =
                                        islandDetailsFromBroadcastPair(islandBroadcastPair);
                            }
                            else{
                                // Store this until we can broadcast
                                anchor_decay.RobotPlayer.myIslandFullInfoBroadcastQueue.add(islandBroadcastPair);
                            }
                        }
                        else {
