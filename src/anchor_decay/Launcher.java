...
//        RobotInfo[] nearby_bots = rc.senseNearbyRobots();
        ...
        int my_task = anchor_decay.Comms.get_command_for_me(rc);

        if(my_task > 0){
            target_location = anchor_decay.Comms.get_MapLocation(my_task);
            int desired_task = anchor_decay.Comms.get_task_type(my_task);
